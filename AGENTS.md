...
- 配置持久化在 `settings.py`：读写 `~/.quant_local_config.json`（权限 0600）。优先级：GUI 显式参数 > JSON > env > config.py 默认。`engine.MonitorEngine.__init__` 自动加载 JSON 并合并 env；`save_runtime_config()` 写回 JSON。GUI「配置中心」Tab 是唯一用户配置入口，env 仅作 TUI 回退。
- `notification.py` 飞书配置支持运行时切换：`configure_feishu(webhook, secret)` 改模块级全局变量，不依赖 `config.py`。`send_feishu` 返回 bool 表示成功与否。
